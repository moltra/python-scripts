...
def _move_to_extension_dir(file_path: Path, base_dir: Path, dry_run: bool) -> None:
    """
    Summary – Move a single file into a subfolder named by its extension.
...
def organize_downloads(folder: Optional[str | Path] = None, dry_run: bool = False) -> None:
    """
    ...
    Uses Loguru for structured logging and includes a dry-run mode for safety.

    @param folder     Directory to organize. If None, uses the user's Downloads folder.
    @param dry_run    When True, print intended actions but do not move files.
    @returns          None
    ...
    @example
      ...
      # Organize a specific path, without making changes
      organize_downloads(r"C:\\Users\\me\\Downloads", dry_run=True)
    """
...
if __name__ == "__main__":
    # Configure Loguru to emit structured JSON to stdout (follows the project's logging rules)
    logger.remove()
    logger.add(sys.stdout, serialize=True, level="INFO")

    # Call with default folder (user's Downloads). Toggle dry_run=True to preview actions.
    organize_downloads()
